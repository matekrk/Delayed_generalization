### Kudos to: https://github.com/kohpangwei/group_DRO
...
cub_dir = '/shared/sets/datasets/vision/CUB-200'
cub_seg_dir = '/shared/sets/datasets/vision/CUB-200_segmentations'
places_dir = '/shared/sets/datasets/vision/places365_standard'
output_dir = '/shared/sets/datasets/vision/waterbirds'
...
for idx, target_places in enumerate(target_places):
    ...
    for target_place in target_places:
        ...
        assert (np.sum(place_ids_df['place_name'] == target_place_full) == 1), \
            f"Place {target_place_full} ERROR: {np.sum(place_ids_df['place_name'] == target_place_full)} matches"
        ...
        place_filenames += [
            f'/{target_place}/{filename}' for filename in os.listdir(
                os.path.join(places_dir, 'train', target_place))
